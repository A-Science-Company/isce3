...
def get_products_and_paths(cfg: dict) -> (dict, dict):
    ...
    product_dict = {'ROFF_RIFG_RUNW_GOFF_GUNW': insar_products,
                    ...
                    'GCOV': ['GCOV'],
                    'GSLC': ['GSLC'],
                    # ASC PATCH: h5_paths below defines RUNW_STANDALONE and
                    # GUNW_STANDALONE, and unwrap.py / geocode_insar.py set
                    # product_type to exactly those strings when run as
                    # `python -m nisar.workflows.unwrap <cfg>`. Without matching
                    # entries here, get_products_and_paths raises
                    # KeyError: 'RUNW_STANDALONE' and the standalone entry
                    # points are unusable.
