        ...
        for freq, pol_list, offset_pol_list in get_cfg_freq_pols(cfg):
            ...
            for pol in pol_list:
                ...
                if algorithm == "icu":
                    ...
                    # --- ASC PATCH: stream inputs instead of loading them ---
                    # Stock code called open_raster(), which materialises the
                    # WHOLE interferogram and coherence as numpy arrays before
                    # snaphu ever runs. On a full-resolution (1x1) frequency A
                    # grid that is 53200 x 54244:
                    #     complex64 interferogram  23.1 GB
                    #     float32   coherence      11.5 GB
                    #                              34.6 GB   -- over a 31 GB box
                    # which made 1x1 unwrapping impossible for reasons that had
                    # nothing to do with snaphu: snaphu.unwrap() takes an
                    # InputDataset Protocol, and snaphu.io.Raster implements it
                    # as a file-backed rasterio dataset that snaphu reads
                    # tile-by-tile. Measured peak RSS for opening both 1x1
                    # rasters this way: 0.10 GB.
                    #
                    # The outputs were already streamed (unw=dst_h5[...]); only
                    # the inputs were not.
                    #
                    # PATH FORM MATTERS. ISCE3 builds "HDF5:<file>:/<group>",
                    # which rasterio rejects; it needs "HDF5:<file>://<group>"
                    # (or the quoted form). _asc_stream() normalises that and
                    # leaves plain ENVI paths -- used when phase_unwrap
                    # re-multilooks -- untouched.
                    def _asc_stream(p):
                        import re as _re, os as _os
                        import snaphu.io as _sio
                        if p is None:
                            return None
                        s = str(p)
                        m = _re.match(r'^HDF5:(.+?):/(?!/)(.+)$', s)
                        if m:
                            s = f"HDF5:{_os.path.abspath(m.group(1))}://{m.group(2)}"
                        return _sio.Raster(s)

                    igram_array = _asc_stream(igram_path)
                    coh_array = _asc_stream(corr_path)
                    mask_array = _asc_stream(snaphu_cfg['mask'])
